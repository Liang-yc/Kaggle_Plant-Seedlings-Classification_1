...
def build_deformable_conv2d_layer(input, training, layer_number, num_filter=32, kernel_size=2,
                                  bn_momentum=0.9, pool_size=2, pool_strides=2, activation_fn=tf.nn.leaky_relu):
    ...
    cnn_out = DeformConv2D(input, kernel_size, num_filter, layer_name, 1, True).infer()
...
class DeformConv2D(object):
        ...
        with tf.name_scope(self.name + "_" + mode):
            ...
            if mode == "offset":
                ...
                with tf.variable_scope(self.name):
                    ...
                    weights = tf.get_variable(name="offset_weights",
                                              shape=[ks, ks, ci, co],
                                              trainable=self.trainable,
                                              initializer=tf.zeros_initializer)

                    # Create the biases
                    biases = tf.get_variable(name="offset_biases",
                                             shape=[co],
                                             trainable=self.trainable,
                                             initializer=tf.zeros_initializer)
    ...
    def infer(self):
        ...
        with tf.name_scope(self.name):
            ...
            layer_output = self.conv(x_offset, self.co, "feature", groups=self.groups, stride=ks, relu=False)
