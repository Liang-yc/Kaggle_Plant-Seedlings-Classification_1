...
TRAIN_CONFIG = {
    ...
    'image_shape': model.get_input_shape(),
...
def test_augmented_acc(linear, y):
    ...
    test_accuracy_op = tf.reduce_mean(
...
def train():
    ...
    linear, logits, trainable_var = model.build_model(x, y, NUM_CLASS,
                                                      lambda_decay, is_training)
    ...
    test_augmented_accuracy_op, test_augmented_confusion_matrix_op = test_augmented_acc(
        linear, y)
    confusion_matrix_op = tf.confusion_matrix(
        y, tf.argmax(linear, 1), num_classes=NUM_CLASS, dtype=tf.int32)
    ...
    with tf.Session(config=session_config) as session:
        ...
        model.load_pretrained_weight(session)
        session.run(hvd.broadcast_global_variables(0))

        # epoch
        training_process(
            accuracy_op, global_step, image_batch, label_batch, is_training,
            loss_op, session, train_op, x, y, args.num_epoch,
...
def training_process(accuracy_op, global_step, image_batch, label_batch,
    ...
    for i in range(num_epoch):
        loss_avg = training_phase(
            accuracy_op, global_step, i, image_batch, label_batch, is_training,
            loss_op, merge_summary, session, summary_writer, train_op, x, y)

        model_saver.save(
            session,
            os.path.join(
                save_path,
                "plant_seedings_classifier_{0:.8f}.ckpt".format(loss_avg)),
    ...
    for image, label in test_data:
        ...
        accuracy_value, confusion = session.run(
            ...
            feed_dict={x: image,
                       ...
                       is_training: False})
        test_accuracy_avg = test_accuracy_avg + (
            accuracy_value - test_accuracy_avg) / (k + 1)

        image_augmented = session.run(
            [x_augmented], feed_dict={x_test: image})[0]
        augmented_accuracy_value, augmented_confusion = session.run(
            [test_augmented_accuracy_op, test_augmented_confusion_matrix_op],
            feed_dict={x: image_augmented,
                       y: label,
                       is_training: False})

        test_augmented_accuracy_avg = test_augmented_accuracy_avg + (
            augmented_accuracy_value - test_augmented_accuracy_avg) / (k + 1)

        k += 1
        confusion_matrix = confusion_matrix + confusion
        augmented_confusion_matrix = augmented_confusion_matrix + augmented_confusion
        sys.stdout.write("\raugmented_test acc:{0} - test acc:{1}           ".
                         format(test_augmented_accuracy_avg, test_accuracy_avg))
        sys.stdout.flush()
    print("")
...
def training_phase(accuracy_op, global_step, epoch, image_batch, label_batch,
    ...
    for j in range(
            int(
                math.ceil(CONFIG_USE[
                    'training_size'] * DATA_AUGMENTATION_TIMES_PER_REPLICA /
