...
def l_softmax_training(input, target, margin, weight):
    ...
    cosm = tf.reduce_sum(cosm_terms, 1, keep_dims=True)
    ...
    logits = [[logit, x, logit, logit],
              ...
              [logit, logit, logit, x]]
    """
    updated = tf.scatter_nd(logits_target_indices, tf.squeeze(ls_target), tf.shape(logits))
    mask = tf.cast(tf.sparse_to_dense(logits_target_indices, tf.shape(logits), 1), tf.bool)
